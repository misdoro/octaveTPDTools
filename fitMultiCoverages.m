function result=fitMultiCoverages(mytpd,param,result);
  ...
  if (isfield(param,"SitesLineStyle"))
    ls=param.SitesLineStyle;
  else
    ls="none";
  endif
  plot(Epts,fitde.thetas,"linestyle",ls,"marker",markers(mytpd.fileidx),"markersize",12,"linewidth",2,"color",mytpd.color);
  ...
  if (index(param.tools,'N'))
    ...
    parr/=(param.monolayer.*(mytpd.rate));
  endif
  figure(getFigIndex("disp"))
  plot(mytpd.T,p,"color",mytpd.color,"linestyle","--");
  
  if (isfield(param,"plotdEcomp"))
    cc=columns(parr)
    for i=1:cc
      compcolor=getLineColor(6*(i/cc),1,2);
      plot(mytpd.T,parr(:,i),"color",compcolor,"linestyle","-.","linewidth",2);
    endfor;
  endif
